#!/usr/bin/env python3
"""
Enhanced Sentence and Word ID Generation Script (Fixed Version)
...
Fixed to handle the actual JSON structure with content.parts array.
"""
...
def process_conversations(input_dir, output_dir):
            ...
            if 'mapping' in data:
                for msg_id, msg_data in data['mapping'].items():
                    if 'message' in msg_data and msg_data['message']:
                        content = extract_message_content(msg_data)
                        if content:
...
def main():
    parser = argparse.ArgumentParser(description='Generate enhanced sentence and word IDs for ChatGPT conversations')
    parser.add_argument('-i', '--input', required=True, help='Input directory containing JSON files')
    parser.add_argument('-o', '--output', required=True, help='Output directory for results')

    args = parser.parse_args()

    print("🔍 Processing conversations for enhanced sentence and word ID generation...")

    summary = process_conversations(args.input, args.output)

    print(f"\n✅ Enhanced sentence and word ID generation complete!")
    print(f"📊 Files processed: {summary['total_files_processed']}")
    print(f"📊 Total sentences: {summary['total_sentences_found']}")
    print(f"📊 Unique sentences: {summary['unique_sentences']}")
    print(f"📊 Total words: {summary['total_words_found']}")
